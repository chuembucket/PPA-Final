...
trips1day <- read.socrata("https://data.sfgov.org/resource/imvp-dq3v.csv?$where=session_start_dt between '2021-06-08T00:00:00' and '2021-06-09T00:00:00'") 



trips <- trips1day %>% mutate(
  ...
  filter(length_secs > 600 & length_secs < 10800  & meter_event_type == 'NS')
...
dat3 <- smalltrips_dt %>% group_by(start_interval15, post_id) %>%
  mutate(tokens = as.numeric(max(length) )/ 900,
...
transaction_panel <- study.panel  %>%
  ...
  mutate(occupancy = ifelse(tokens_00 + lag01 + lag02 + lag03+ lag04 + lag05 +
           lag06 + lag07 + lag08 + lag09 + lag10 + lag11 + lag12 + lag13+lag14+lag15+lag16+lag17+lag18+lag19+lag20+lag21+lag22+lag23+lag24 >= 1, 1, 0))
...
transaction_panel3 <- transaction_panel %>% left_join(meterblock, by = c("post_id", "street_block")) %>% 
  relocate(occupancy, street_block, .after = doty) %>% filter(street_block != 0)



transaction_panel2 <- transaction_panel3 %>% select(start_interval15, post_id, occupancy, street_block) %>% 
  mutate(start_interval15 = start_interval15 %>% as_datetime())

transaction_panel1 <- left_join(transaction_panel2, smalltrips_dt, by = c("start_interval15", "street_block")) %>% left_join(metercount, by = 'street_block')



occ_panel <- transaction_panel1 %>% 
  group_by(street_block, start_interval15) %>% summarise(
  parked = sum(occupancy),
  total_spots = metercount,
...
```{r}
ggplot(occ_panel)+
  geom_histogram(aes(x=pct_occ, fill = hour))+
...
mapanim <- ggplot(anim_panel, aes(color = occ_col))+
  ...
  labs(title = "Block Occupancy Percent on June 7th",
    ...
    mapTheme()
anim_save("map1.gif", mapanim, duration = 10)
